...
typedef struct 
{
    ...
    HI_BOOL bProgRevise;
    HI_BOOL bVdecCreate;/**CNcomment:HI_TRUE,表示VDEC创建,HI_FALSE,表示OMXVDEC创建*/
...
typedef enum
{
    HI_DRV_VPSS_BYPASSMODE_AUTO = 0,    /*透传与否由VPSS决策*/
    HI_DRV_VPSS_BYPASSMODE_ENABLE,      /*PORT透传使能*/
    HI_DRV_VPSS_BYPASSMODE_DISABLE,     /*PORT不透传禁止*/
    HI_DRV_VPSS_BYPASSMODE_BUTT
}HI_DRV_VPSS_BYPASSMODE_E;
...
typedef struct
{
    ...
    HI_BOOL   b3Dsupport;
    HI_DRV_VPSS_BYPASSMODE_E   enByPassMode;
    
    HI_DRV_VPSS_ROTATION_E enRotation;
    HI_BOOL bVertFlip;
    HI_BOOL bHoriFlip;
    HI_BOOL b3DDetectPort;
...
typedef struct
{
    HI_U32                      u32InputWidth;        /*VPSS判断是否透传的条件*/
    HI_U32                      u32InputHeight;
    HI_DRV_PIX_FORMAT_E         enInputPixFormat;
    HI_U32                      u32InputFrameRate;
    HI_BOOL                     bVc1;
    HI_BOOL                     bForceBypass;
    HI_BOOL                     bVpssBypass;          /*VPSS返回的结果*/
...
typedef enum
{
    ...
    HI_DRV_VPSS_USER_COMMAND_SETPAUSE,
