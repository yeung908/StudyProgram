...
typedef struct hiVDEC_VIDEO_USERDATA_S
{
    ...
    HI_VIRT_ADDR_T                     pu8Buffer;           /**<Initial address of the user data memory, output parameter*/ /**<CNcomment: 用户数据内存区的初始地址,输出参数*/
...
{
    HI_HANDLE               		hHandle;
    HI_UNF_AVPLAY_I_FRAME_64BIT_S   stIFrame;
    HI_DRV_VIDEO_FRAME_S 			stVoFrameInfo;
    HI_BOOL                 		bCapture;
...
typedef struct hiVDEC_CMD_USERDATAFREE_S
{
    HI_HANDLE               hHandle;        
    USERDATA_TYPE_INDEX_E   enIndex;
}VDEC_CMD_USERDATAFREE_S;
...
{
    HI_VIRADDR u32StartVirAddr;
    HI_PHYADDR u32StartPhyAddr;
    HI_U32 u32Size;
...
typedef struct hiVDEC_BUFFER_INFO_64BIT_S
{
    HI_U32 u32PhyAddr;
    VDEC_FRAMEBUFFER_STATE_E enBufState;
}VDEC_BUFFER_INFO_64BIT_S;
typedef struct hiVDEC_CMD_CHECKANDDELBUFFER_S
{
    HI_HANDLE                       hHandle;
    VDEC_BUFFER_INFO_64BIT_S        stBufInfo;
...
typedef struct hiVDEC_CMD_SET_PAUSE_STATUS_S
{
    HI_HANDLE                      hVdecHandle;
    HI_BOOL                        bEnable;
}VDEC_CMD_SET_PAUSE_STATUS_S;
/**3D使能/去使能
 **/
typedef struct hiVDEC_CMD_FDMNG_ENABLE_S
{
    HI_HANDLE                      hHandle;
    HI_BOOL                        bEnable;
}VDEC_CMD_FDMNG_ENABLE_S;

/**3D模式查询
 **/
typedef struct hiVDEC_FDMNG_QUERYRESULT_S
{
    HI_HANDLE                      hHandle;
    HI_DRV_FRAME_TYPE_E 		   stEframeType;
}VDEC_FDMNG_QUERYRESULT_S;

typedef struct hiVDEC_CMD_VP9_FRMBUF_STATUS_S
{
   HI_HANDLE               hHandle;
   VDEC_USR_BUFFER_INFO_S   stBufInfo;
}VDEC_CMD_VP9_FRMBUF_STATUS_S;
typedef struct hiVDEC_CMD_VP9_GET_FRAME_INFO_S
{
    HI_HANDLE               hHandle;
    HI_DRV_VDEC_VP9_FRAME_BUF_INFO_S   stFrmBuf;
}VDEC_CMD_VP9_GET_FRAME_INFO_S;

typedef struct hiVDEC_CMD_DFS_SET_PARAM_S
{
    HI_HANDLE               hHandle;
    HI_DRV_VDEC_DFS_PARAM_S stDFSParam;
}VDEC_CMD_DFS_SET_PARAM_S;

typedef struct hiVDEC_CMD_DFS_GET_FRAME_STATE_S
{
    HI_HANDLE               hHandle;
    HI_U32                  u32State; 
}VDEC_CMD_DFS_GET_FRAME_STATE_S;
...
#define UMAPC_VDEC_CHAN_IFRMDECODE            _IOWR(HI_ID_VDEC, 0x85, VDEC_CMD_IFRAME_DEC_64BIT_S)
...
#define UMAPC_VDEC_CHAN_USERDATADEINITBUF     _IOWR(HI_ID_VDEC, 0x8f, VDEC_CMD_USERDATAFREE_S)
#define UMAPC_VDEC_CHAN_USERDATASETBUFADDR    _IOW(HI_ID_VDEC,  0x90, VDEC_CMD_BUF_USERADDR_S)
#define UMAPC_VDEC_CHAN_ACQUSERDATA           _IOWR(HI_ID_VDEC, 0x91, VDEC_CMD_USERDATA_S)
#define UMAPC_VDEC_CHAN_RLSUSERDATA           _IOW(HI_ID_VDEC,  0x92, VDEC_CMD_USERDATA_S)

#define UMAPC_VDEC_CHAN_SETCTRLINFO           _IOW(HI_ID_VDEC,  0x93, VDEC_CMD_SET_CTRL_INFO_S)
#define UMAPC_VDEC_CHAN_PROGRSSIVE            _IOW(HI_ID_VDEC,  0x94, VDEC_CMD_SET_PROGRESSIVE_S)
#define UMAPC_VDEC_CHAN_DPBFULL               _IOWR(HI_ID_VDEC, 0x95, VDEC_CMD_SET_DPBFULL_CTRL_S)
...
#define UMAPC_VDEC_FDMNG_ENABLE               _IOW(HI_ID_VDEC,  0xc6, VDEC_CMD_FDMNG_ENABLE_S)
#define UMAPC_VDEC_FDMNG_QUERYRESULT          _IOWR(HI_ID_VDEC, 0xc7, VDEC_FDMNG_QUERYRESULT_S)

// add by w00278582
#define UMAPC_VDEC_VPU_PROC                   _IOWR(HI_ID_VDEC, 0xc8, VDEC_CMD_VPU_PROC_STATUS_S)

#define UMAPC_VDEC_PAUSE                      _IOWR(HI_ID_VDEC, 0xc9, VDEC_CMD_SET_PAUSE_STATUS_S)
#define UMAPC_VDEC_RESUME                     _IOWR(HI_ID_VDEC, 0xca, VDEC_CMD_SET_PAUSE_STATUS_S)
...
#define UMAPC_VDEC_CHAN_GET_VP9_FRAME_INFO    _IOWR(HI_ID_VDEC, 0xd5, VDEC_CMD_VP9_GET_FRAME_INFO_S)
#define UMAPC_VDEC_CHAN_CHECKFRMSTATUS        _IOWR(HI_ID_VDEC, 0xd6, VDEC_CMD_VP9_FRMBUF_STATUS_S)
#define UMAPC_VDEC_CHAN_DFS_SET_PARAM         _IOWR(HI_ID_VDEC, 0xd7, VDEC_CMD_DFS_SET_PARAM_S)
#define UMAPC_VDEC_CHAN_DFS_GET_FRAME_BUF_STATE _IOWR(HI_ID_VDEC, 0xd8, VDEC_CMD_DFS_GET_FRAME_STATE_S)
#ifdef __cplusplus
#if __cplusplus
}
#endif
