...
typedef struct USRMODEPROC_IOCTL_ENTRY_S     /* 兼容32位与64位用于用户态与内核态的调用*/
{
    HI_CHAR aszName[MAX_PROC_NAME_LEN + 1];     /* Input, entry name */
    HI_CHAR aszParent[MAX_PROC_NAME_LEN + 1];   /* Input, directory name */
    HI_VIRT_ADDR_T u32fnShowFuncAddr;           /* Input, show function address */
    HI_VIRT_ADDR_T u32fnCmdFuncAddr;            /* Input, cmd function address*/
    HI_VIRT_ADDR_T u32PrivDataAddr;             /* Input, private data address*/
    HI_VIRT_ADDR_T u32EntryAddr;                /* Output, entry pointer address*/
    HI_VIRT_ADDR_T u32FileAddr;                 /* Output, Belongs to which file address */
    MMZ_BUFFER_S stBuf;                         /* Output, buffer */
}HI_DRV_USRMODEPROC_IOCTL_ENTRY_S;

typedef struct USRMODEPROC_IOCTL_CMD_S       /* 兼容32位与64位用于用户态与内核态的调用*/
{
    HI_VIRT_ADDR_T u32EntryAddr;               /* The type is HI_DRV_USRMODEPROC_ENTRY_S address* */
    HI_S32      s32Write;
    HI_CHAR     aszCmd[MAX_PROC_CMD_LEN + 1];
}HI_DRV_USRMODEPROC_IOCTL_CMD_S;

typedef struct USRMODEPROC_IOCTL_CMDINFO_S  /* 兼容32位与64位用于用户态与内核态的调用*/
{
    HI_DRV_USRMODEPROC_IOCTL_CMD_S stCmd;
    HI_DRV_USRMODEPROC_IOCTL_ENTRY_S stEntry;
}HI_DRV_USRMODEPROC_IOCTL_CMDINFO_S;
...
#define	UMPIOC_ADD_ENTRY		           _IOWR(HI_ID_PROC, 1, HI_DRV_USRMODEPROC_IOCTL_ENTRY_S)
#define	UMPIOC_REMOVE_ENTRY		    _IOW(HI_ID_PROC, 2, HI_DRV_USRMODEPROC_IOCTL_ENTRY_S)
#define	UMPIOC_ADD_DIR		           _IOW(HI_ID_PROC, 3, HI_PROC_DIR_NAME_S)
#define	UMPIOC_REMOVE_DIR                 _IOW(HI_ID_PROC, 4, HI_PROC_DIR_NAME_S)
#define	UMPIOC_GETCMD   		           _IOR(HI_ID_PROC, 5, HI_DRV_USRMODEPROC_IOCTL_CMDINFO_S)
