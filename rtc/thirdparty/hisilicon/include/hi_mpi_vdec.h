...
#ifndef HI_VDEC_SVP9_SUPPORT
#define HI_VDEC_SVP9_SUPPORT (0)
#endif

#ifndef HI_VDEC_USERDATA_CC_SUPPORT
#define HI_VDEC_USERDATA_CC_SUPPORT (0)   //调试需要，暂时先与CC字幕共用一个外部cfg配置,该宏的意义有变，实际是指支持UsrDate数据的发送
#endif

#define HI_VDEC_SUPPORT_USRDATA_TYPE (2) //目前仅分成两类: CC/AFD

#define MAX_VDEC_EXT_BUF_NUM 16

#ifndef CONFIG_ARCH_LP64_MODE
    #define HI_VIRADDR HI_U32
#else
    #define HI_VIRADDR HI_U64
#endif
#define HI_PHYADDR HI_U32

/*************************** Structure Definition ****************************/

typedef struct hiVDEC_ES_BUF_S
{
    HI_VIRADDR   pu8Addr;      /*Virtual address of the buffer user.*/
...
{
    HI_U32 u32UsrVirAddr[MAX_VDEC_EXT_BUF_NUM];  /**<User virtual address *//**<CNcomment: 用户态虚拟地址*/
...
typedef enum hiVDEC_USR_FRAMEBUFFER_STATE_E
{
    VDEC_USR_BUF_STATE_EMPTY = 0,
    VDEC_USR_BUF_STATE_IN_USE,
    VDEC_USR_BUF_STATE_BUTT
}VDEC_USR_FRAMEBUFFER_STATE_E;
typedef struct hi_VDEC_USR_BUFFER_INFO_S
{
    HI_U32 u32PhyAddr;
    VDEC_USR_FRAMEBUFFER_STATE_E eUsrBufState;
} VDEC_USR_BUFFER_INFO_S;
...
/*new api,for both CC and AFD UsrData*/
HI_S32 HI_MPI_VDEC_AcqUserDataExt(HI_HANDLE hVdec,HI_UNF_VIDEO_USERDATA_TYPE_E enType,
                HI_UNF_VIDEO_USERDATA_S* pstUserData);
HI_S32 HI_MPI_VDEC_RlsUserDataExt(HI_HANDLE hVdec,HI_UNF_VIDEO_USERDATA_TYPE_E enType, 
	            HI_UNF_VIDEO_USERDATA_S* pstUserData);
...
HI_S32 HI_MPI_VDEC_Pause(HI_HANDLE hVdec);
HI_S32 HI_MPI_VDEC_Resume(HI_HANDLE hVdec);
