...
#define HA_FATAL_PRINT
#define HA_ERR_PRINT
#define HA_WARN_PRINT
#define HA_INFO_PRIN
#define HA_DBG_PRINT
...
typedef struct
{
    ...
    HI_BOOL bHwCodec;         /**<HI_TRUE: hwdecode mode else swdecode mode. */
...
typedef struct
{
    HI_U32  enCmd;              /**<set outbuf addr command.*/ /**<CNcomment: 设置输出Buffer地址命令*/
    HI_U32  u32OutBufAddr;         /**<outbuf addr. */  /**<CNcomment:输出Buffer地址*/
} HA_CODEC_OUTBUFADDR_SET_PARAM_S;
...
{
    HI_U32   u32BufAddr;             /**<Output, buffer addr*/ /**<CNcomment: 缓存地址*/
    HI_U32   u32BufSize;                /**<Input, buffer size*/ /**<CNcomment: 缓存大小*/
    HI_U32   u32PtsBoundary;            /**<Input, pts read pointers wrap*/ /**<CNcomment:pts读指针边界 */
} HI_HA_INPUTBUF_S;

/**Hardware decode input buffer process attribute.*/
/**CNcomment:硬解输入缓存处理属性*/
typedef struct hiHI_HA_INBUF_PROCESS_S
{
    HI_U32   u32BufOffsetAddr1;             /**<Output, buffer offset addr 1*/ /**<CNcomment: 缓存偏移地址1*/
    HI_U32   u32BufOffsetAddr2;             /**<Output, buffer offset addr 2*/ /**<CNcomment: 缓存偏移地址2*/
    HI_U32   u32BufRequestSize1;            /**<Output, buffer requeset size 1*/ /**<CNcomment:缓存大小1*/
    HI_U32   u32BufRequestSize2;            /**<Output, buffer requeset size 2*/ /**<CNcomment: 缓存大小2*/
    HI_U32   u32RequestSize;                /**<Input, requeset buffer size*/ /**<CNcomment: 请求的缓存大小*/
} HI_HA_INBUF_PROCESS_S;
...
{
    HI_U32   u32BufAddr;              /**<Output, buffer addr*/ /**<CNcomment: 缓存地址*/
    HI_U32   u32BufSize;              /**<Output, buffer size*/ /**<CNcomment: 缓存大小*/
    HI_U32   u32MaxBufSizePerFrame;      /**<Input, max frame size*/ /**<CNcomment: 每帧的缓存大小*/
    HI_U32   u32OutFrameNum;             /**<Input, max frame number*/ /**<CNcomment: 总帧数*/
} HI_HA_OUTPUTBUF_S;

/**Hardware decode status information.*/
/**CNcomment:硬解状态信息*/
typedef struct hiHI_HA_STATUS_INFO_S
{
    HI_U32      u32InBufReadPos;   /**<Output, input buffer read position*/ /**<CNcomment: 输入缓存读位置*/
    HI_U32      u32InBufWritePos;  /**<Output, input buffer write position*/ /**<CNcomment: 输入缓存写位置*/

    HI_U32      u32OutBufReadIdx;  /**<Output, output buffer read index, 0~N-1, N is output buffer total frame number*/ 
                                   /**<CNcomment: 输出缓存读位置下标索引，0~N-1，N为输出缓存总帧数*/
    HI_U32      u32OutBufReadWrap; /**<Output, output buffer read wrap, 0 or 1*/ /**<CNcomment: 输出缓存读下标回绕标志，0或者1*/
    HI_U32      u32OutBufWriteIdx; /**<Output, output buffer write index, 0~N-1, N is output buffer total frame number*/ 
                                   /**<CNcomment: 输出缓存写位置下标索引，0~N-1，N为输出缓存总帧数*/
    HI_U32      u32OutBufWriteWrap;/**<Output, output buffer write wrap, 0 or 1*/ /**<CNcomment: 输出缓存写下标回绕标志，0或者1*/

    HI_U32      u32TryExeCnt;      /**<Output, try decode count*/ /**<CNcomment: 尝试解码次数*/
    HI_U32      u32FrameNum;       /**<Output, decode correct frame count*/ /**<CNcomment: 解码出正确帧的个数*/
    HI_U32      u32ErrFrameNum;    /**<Output, decode incorrect frame count*/ /**<CNcomment: 解码出错误帧的个数*/
    HI_BOOL     bEndOfFrame;       /**<Output, the last frame or not.*/ /**<CNcomment:是否最后一帧*/

    HI_U32      u32ExeTimeOutCnt;  /**<Output, execute timeout count.*/ /**<CNcomment:执行超时次数*/
    HI_U32      u32ScheTimeOutCnt; /**<Output, schedule timeout count.*/ /**<CNcomment:调度超时次数*/

    HI_U32      u32StreamReadPos;  /**<Output, stream virtual read position based on boundary.*/ /**<CNcomment:基于boundary的码流读位置*/
} HI_HA_STATUS_INFO_S;
#endif
...
typedef struct hiHA_DECODE_S
{
    ...
    */
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*HwGetBuf)(HI_VOID* hDecoder,
                                  HI_HA_INBUF_PROCESS_S* pstGetBuf);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*HwPutBuf)(HI_VOID* hDecoder,
                                  HI_HA_INBUF_PROCESS_S* pstPutBuf);
    ...
    */
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*HwSetEosFlag)(HI_VOID* hDecoder,
                                        HI_BOOL bEosOfFlag);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*HwGetStatusInfo)(HI_VOID* hDecoder,
                                         HI_HA_STATUS_INFO_S* pstStatusInfo);
#endif
...
#ifdef HA_HW_CODEC_SUPPORT
    HI_U32  u32PtsReadPos;          /**<Hardware encode pts read position .*/ /**<CNcomment:硬编码pts读位置*/
#endif
...
typedef struct hiHA_ENCODE_S
{
    ...
    */
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*EncodeHwSetEnable)(HI_VOID* hEncoder, HI_BOOL bEnable);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*EncodeHwInitInbuf)(HI_VOID* hEncoder,
                                           HI_HA_INPUTBUF_S* pstInBufAttr);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*EncodeHwDeInitInbuf)(HI_VOID* hEncoder, HI_U32 u32BufAddr);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*EncodeHwInitOutbuf)(HI_VOID* hEncoder,
                                            HI_HA_OUTPUTBUF_S* pstOutBufAttr);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*EncodeHwDeInitOutbuf)(HI_VOID* hEncoder, HI_U32 u32BufAddr);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*EncodeHwGetBuf)(HI_VOID* hEncoder,
                                        HI_HA_INBUF_PROCESS_S* pstGetBuf);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*EncodeHwPutBuf)(HI_VOID* hEncoder,
                                        HI_HA_INBUF_PROCESS_S* pstPutBuf);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*EncodeHwReceiveStream)(HI_VOID* hEncoder,
                                        HI_HAENCODE_OUTPUT_S* pstAOut);
    ...
    N/A
    */
    HI_HA_ERRORTYPE_E (*EncodeHwReleaseStream)(HI_VOID* hEncoder);
#endif
} HI_HA_ENCODE_S;
