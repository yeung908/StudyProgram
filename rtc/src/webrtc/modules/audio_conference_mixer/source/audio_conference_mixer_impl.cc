    ...
    size_t numMixedParticipants;
    {
        ...
        if(mixable) {
            WEBRTC_TRACE(kTraceInfo, kTraceAudioMixerServer, _id,
                "AddParticipantToList point:%p", participant);
            success = AddParticipantToList(participant, &_participantList);
        } else {
            WEBRTC_TRACE(kTraceInfo, kTraceAudioMixerServer, _id,
                "RemoveParticipantFromList point:%p", participant);
    ...
    for (AudioFrameList::const_iterator iter = audioFrameList.begin();
        ...
        if (!iter->muted) {
          MixFrames(mixedAudio, iter->frame, use_limiter_);
            ...
            WEBRTC_TRACE(kTraceStream, kTraceAudioMixerServer, _id, "frame is muted");
        }
