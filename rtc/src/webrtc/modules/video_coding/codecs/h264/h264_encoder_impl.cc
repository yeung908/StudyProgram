...
#include "thirdparty/openh264/codec/api/svc/codec_api.h"
#include "thirdparty/openh264/codec/api/svc/codec_app_def.h"
#include "thirdparty/openh264/codec/api/svc/codec_def.h"
#include "thirdparty/openh264/codec/api/svc/codec_ver.h"
...
int32_t H264EncoderImpl::InitEncode(const VideoCodec* codec_settings,
  ...
  LOG(INFO) << "init openh264 " << (codec_settings->mode == kScreensharing ? "screen" : "camera") << " encoder " << "width:" << codec_settings->width << " height:" << codec_settings->height\
  ...
  stream << tempPath <<"maxhub_openh264_"<<(codec_settings->mode == kScreensharing ? "screen" : "camera")<<"_" << timebuffer << width_ << "_" << height_ << ".264";
...
int32_t H264EncoderImpl::SetRateAllocation(
  ...
  if (bitrate_allocation.get_sum_bps() <= 0 || framerate <= 0)
    return WEBRTC_VIDEO_CODEC_ERR_PARAMETER;
  LOG(INFO) << "modify openh264 " << (mode_ == kScreensharing ? "screen" : "camera") << " encode bitrate " << bitrate_allocation.get_sum_bps();
...
int32_t H264EncoderImpl::Encode(const VideoFrame& input_frame,
  ...
  if (dumpfile_)
  {
      LOG(INFO) << "openh264 encode frame cost:" << GetTickCount() - t1 << " ms, RtpFragmentize:" << encoded_image_._length;
