...
NackModule::NackInfo::NackInfo()
    : seq_num(0), send_at_seq_num(0), sent_at_time(-1), recv_at_time(-1), retries(0) {}
...
int NackModule::OnReceivedPacket(const VCMPacket& packet) {
  ...
  //for (auto nackseq : nack_batch)
  //{
  //    LOG(LS_WARNING) << "ask nack for seq :" << nackseq;
  //}
...
void NackModule::Process() {
  ...
  //for (auto nackseq : nack_batch)
  //{
  //    LOG(LS_WARNING) << "ask nack for seq :" << nackseq << " now :" << now_ms;
  //}
}
...
void NackModule::AddPacketsToNack(uint16_t seq_num_start,
  ...
  for (uint16_t seq_num = seq_num_start; seq_num != seq_num_end; ++seq_num) {
    ...
    nack_info.recv_at_time = now_ms;
    nack_list_[seq_num] = nack_info;
    //LOG(LS_WARNING) << "seq : " << seq_num << "recv at : " << nack_info.recv_at_time;
  }
}
...
std::vector<uint16_t> NackModule::GetNackBatch(NackFilterOptions options) {
  ...
  while (it != nack_list_.end()) {
    if (consider_seq_num && it->second.sent_at_time == -1 &&
        ...
        //LOG(LS_WARNING) << "recv " << it->second.recv_at_time << " now " << now_ms;
        if (it->second.recv_at_time + kProcessIntervalMs <= now_ms)
        {
