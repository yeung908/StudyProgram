...
#ifndef WEBRTC_MODULES_VIDEO_CAPTURE_MAIN_ANDROID_SURFACETEXTUREHELPER_JNI_H_
#define WEBRTC_MODULES_VIDEO_CAPTURE_MAIN_ANDROID_SURFACETEXTUREHELPER_JNI_H_
...
class SurfaceTextureHelper : public rtc::RefCountInterface {
 public:
  ...
  static rtc::scoped_refptr<SurfaceTextureHelper> create(jstring thread_name, jobject j_egl_context);
 ...
 protected:
  ...
  SurfaceTextureHelper(jstring thread_name, jobject j_egl_context);

 private:
  std::unique_ptr<GlobalRef> j_surface_texture_helper_;
  jmethodID j_return_texture_method_;
  jmethodID j_dispose_method_;
  std::unique_ptr<NativeRegistration> j_native_registration;
};
