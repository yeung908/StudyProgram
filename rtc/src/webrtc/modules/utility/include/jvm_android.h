...
class GlobalRef {
 public:
  ...
  const jobject GetRawObject() const {return j_object_;}
...
class JavaClass {
 public:
  ...
  jfieldID GetFieldId(const char* field, const char* signature);
  jobject CallStaticObjectMethod(jmethodID methodID, ...);
  jint CallStaticIntMethod(jmethodID methodID, ...);
  jclass GetClass() const {return j_class_;}
...
class JVM {
 public:
  ...
  void SetEGLContext(jobject local);

  jobject localEGLContext() const {return localEglContext_;}
