...
void NackTracker::UpdateLastDecodedPacket(uint16_t sequence_number,
  ...
  if (IsNewerSequenceNumber(sequence_number, sequence_num_last_decoded_rtp_) ||
    ...
    any_rtp_decoded_ = true;
  } else if (sequence_number == sequence_num_last_decoded_rtp_) {
      ...
      LOG(LS_WARNING) << "NackTracker error ocurred. sequence_number = "
          << sequence_number << "sequence_num_last_decoded_rtp_ =" << sequence_num_last_decoded_rtp_;
      Reset();
  }
