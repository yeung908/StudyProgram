...
class SocketFactory : public UdpTransportImpl::SocketFactoryInterface {
 public:
  UdpSocketWrapper* CreateSocket(const int32_t id,
      ...
      UdpSocketWrapper *socket = UdpSocketWrapper::CreateSocket(id, mgr, obj, cb, ipV6Enable,
                                          disableGQOS);
      if (socket) {
          int32_t val = 1;
          if (!socket->SetSockopt(
              SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<int8_t*>(&val), sizeof(val))) {
              WEBRTC_TRACE(kTraceStream, kTraceTransport, id,"set SO_REUSEADDR failed.");
          }
      }
