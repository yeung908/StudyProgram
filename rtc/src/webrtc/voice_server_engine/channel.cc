...
namespace webrtc
{
    namespace vose
    {
        ...
        class VosERtcpObserver : public RtcpBandwidthObserver
        {
        public:
            explicit VosERtcpObserver(Channel* owner) : owner_(owner) {}
            virtual ~VosERtcpObserver() {}
                ...
                int64_t now_ms) override
            {
                ...
                if (weighted_fraction_lost < 0)                     {
                    weighted_fraction_lost = 0;
                    WEBRTC_TRACE(kTraceStream, kTraceVoice, 0,
                        "VosERtcpObserver::OnReceivedRtcpReceiverReport(weighted_fraction_lost = %d)",
                        weighted_fraction_lost);
                }
            ...
            AudioFrame* audioFrame)
        {
...

            {
                ...
                if (_recvFileRecording) {
                    recv_file_recorder_->RecordAudioToFile(*audioFrame);
                }
            }
...
//             }
            ...
            // Record playout if enabled
            {
                ...
                if (_recvFileRecording && recv_file_recorder_) {
                    recv_file_recorder_->RecordAudioToFile(*audioFrame);
                }
            }
            ...
            _recvAudioLevel.ComputeLevel(*audioFrame);
...
//                 rtc::CritScope cs(&_fileCritSect);
//                 if (send_file_recorder_) {
//                     if (send_file_recorder_->Frequency() > highestNeeded) {
//                         highestNeeded = send_file_recorder_->Frequency();
//                     }
//                 }
//             }
        ...
        void Channel::PlayFileEnded(int32_t id)
        {
            ...
            assert(id == _localFilePlayerId);
            _localFileSending = false;
        ...
        void Channel::RecordFileEnded(int32_t id)
        {
            ...
            if (id == _recvFileRecorderId) {
                _recvFileRecording = false;
            } else if (id == _sendFileRecorderId) {
                _sendFileRecording = false;
            }
        ...
        Channel::Channel(int32_t channelId,
            ...
            _serverConferenceMixerPtr(NULL),
            _recvAudioLevel(),
            _externalTransport(false),
            // Avoid conflict with other channels by adding 1024 - 1026,
            // won't use as much as 1024 channels.
            _localFilePlayerId(VoEModuleId(instanceId, channelId) + 1024),
            _sendFileRecorderId(VoEModuleId(instanceId, channelId) + 1025),
            _recvFileRecorderId(VoEModuleId(instanceId, channelId) + 1026),
            _localFileSending(false),
            _sendFileRecording(false),
            _recvFileRecording(false),
...
//             _outputExternalMediaCallbackPtr(NULL),
            ...
            rtcp_observer_(new VosERtcpObserver(this)),
...
//             }
            ...
            {
                rtc::CritScope cs(&_fileCritSect);
                if (local_file_player_) {
                    local_file_player_->RegisterModuleFileCallback(NULL);
                    local_file_player_->StopPlayingFile();
                }
                if (send_file_recorder_) {
                    send_file_recorder_->RegisterModuleFileCallback(NULL);
                    send_file_recorder_->StopRecording();
                }
                if (recv_file_recorder_) {
                    recv_file_recorder_->RegisterModuleFileCallback(NULL);
                    recv_file_recorder_->StopRecording();
                }
            }
        ...
        int32_t Channel::SetEngineInformation(voe::Statistics& engineStatistics,
        ...
        int32_t Channel::SetServerConferenceMixer(ServerConferenceMixer *mixer)
        {
            if (!_serverConferenceMixerPtr) {
                _serverConferenceMixerPtr = mixer;
                return 0;
            }
        ...
        int32_t Channel::RemoveFromServerConferenceMixer()
        {
            if (_serverConferenceMixerPtr) {
                StopRecv();
                StopSend();
        ...
        int32_t Channel::StartRecv()
        {
            ...
            if (!_externalMixing && _serverConferenceMixerPtr) {
                // Add participant as candidates for mixing.
                if (_serverConferenceMixerPtr->AddChannel(_channelId) != 0) {
                    _engineStatisticsPtr->SetLastError(
            ...
            channel_state_.SetRecving(true);
...
//                 return -1;
        ...
        int32_t Channel::StopRecv()
        {
            ...
            if (!_externalMixing && _serverConferenceMixerPtr) {
                ...
                if (_serverConferenceMixerPtr->RemoveChannel(_channelId) != 0) {
                    _engineStatisticsPtr->SetLastError(
                        VE_AUDIO_CONF_MIX_MODULE_ERROR, kTraceError,
            ...
            channel_state_.SetRecving(false);
            _recvAudioLevel.Clear();
        ...
        int32_t Channel::RegisterVoiceEngineObserver(VoiceServerEngineObserver& observer)
        {
            WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, _channelId),
            ...
            const SdpAudioFormat& format)
        {
            ...
            if (channel_state_.Get().recving) {
                _engineStatisticsPtr->SetLastError(
                    ...
                    "SetRecPayloadType() unable to set PT while recving");
                return -1;
            }
        ...
        int Channel::StartSendingFileLocally(const char fileNameUTF8[1024], bool loop /*= false*/, FileFormats format /*= kFileFormatPcm16kHzFile*/, float volumeScaling /*= 1.0*/, int startPointMs /*= 0*/, int stopPointMs /*= 0*/)
        {
            rtc::CritScope cs(&_fileCritSect);
            ...
            if (local_file_player_) {
                local_file_player_->StopPlayingFile();
                local_file_player_->RegisterModuleFileCallback(NULL);
                local_file_player_.reset();
            } 

            local_file_player_ = FilePlayer::CreateFilePlayer(_localFilePlayerId, format);
            if (!local_file_player_) {
                return -1;
            }

            if (-1 == local_file_player_->StartPlayingFile(fileNameUTF8, loop, startPointMs,
                volumeScaling, 0, stopPointMs, NULL)) {
                _engineStatisticsPtr->SetLastError(VE_BAD_FILE, kTraceError,
                    "StartSendingFileLocally() failed to start file sending");
                local_file_player_->StopPlayingFile();
                local_file_player_.reset();
                return -1;
            }

            local_file_player_->RegisterModuleFileCallback(this);

            return 0;
        }


        int Channel::StartSendingFileLocally(InStream* stream, FileFormats format /*= kFileFormatPcm16kHzFile*/, float volumeScaling /*= 1.0*/, int startPointMs /*= 0*/, int stopPointMs /*= 0*/)
        {
            rtc::CritScope cs(&_fileCritSect);
            ...
            if (0 != local_file_player_->StartPlayingFile(stream, startPointMs,
                volumeScaling, 0, stopPointMs, NULL)) {
                _engineStatisticsPtr->SetLastError(VE_BAD_FILE, kTraceError,
                    "StartSendingFileLocally() failed to start file sending");
                local_file_player_->StopPlayingFile();
        ...
        int Channel::StopSendingFileLocally()
        {
            rtc::CritScope cs(&_fileCritSect);

            if (_localFileSending) {
                local_file_player_->StopPlayingFile();
                local_file_player_->RegisterModuleFileCallback(NULL);
                local_file_player_.reset();
                _localFileSending = false;
        ...
        int Channel::IsSendingFileLocally()
        {
            rtc::CritScope cs(&_fileCritSect);
            return (local_file_player_&&local_file_player_->IsPlayingFile()) ? 0 : -1;
        }


        int Channel::StartRecordingSend(const char* fileNameUTF8, CodecInst* codecInst /*= NULL*/, int maxSizeBytes /*= -1*/)
        {
            if (_sendFileRecording) {
                return 0;
            }
            ...
            if ((codecInst != NULL) &&
                ...
                _engineStatisticsPtr->SetLastError(
                    ...
                    "StartRecordingSend() invalid compression");
                return (-1);
            }
            if (codecInst == NULL) {
            ...
            if (send_file_recorder_) {
                send_file_recorder_->RegisterModuleFileCallback(NULL);
                send_file_recorder_.reset();
            }

            send_file_recorder_ = FileRecorder::CreateFileRecorder(
                _sendFileRecorderId, kFileFormatPcm16kHzFile);
            if (!send_file_recorder_) {
                _engineStatisticsPtr->SetLastError(VE_CANNOT_START_RECORDING, kTraceError,
                    "StartRecordingSend() CreateFileRecorder failed.");
                return -1;
            }

            if (0 != send_file_recorder_->StartRecordingAudioFile(
                fileNameUTF8, *codecInst, notificationTime)) {
                _engineStatisticsPtr->SetLastError(
                    VE_BAD_FILE, kTraceError,
                    "StartRecordingSend() failed to start file recording");
                send_file_recorder_->StopRecording();
                send_file_recorder_.reset();
                return -1;
            }

            send_file_recorder_->RegisterModuleFileCallback(this);
            _sendFileRecording = true;
            return 0;
        }

        int Channel::StartRecordingSend(OutStream* stream, CodecInst* codecInst /*= NULL*/)
        {
            if (_sendFileRecording) {
                return 0;
            }
        ...
        int Channel::StartRecordingRecv(const char* fileNameUTF8, CodecInst* codecInst /*= NULL*/, int maxSizeBytes /*= -1*/)
        {
            if (_recvFileRecording) {
                return 0;
            }
            ...
            if ((codecInst != NULL) &&
                ...
                _engineStatisticsPtr->SetLastError(
                    VE_BAD_ARGUMENT, kTraceError,
                    "StartRecordingRecv() invalid compression");
                return (-1);
            }
            if (codecInst == NULL) {
                format = kFileFormatPcm16kHzFile;
            ...
            if (recv_file_recorder_) {
                recv_file_recorder_->RegisterModuleFileCallback(NULL);
                recv_file_recorder_.reset();
            }

            recv_file_recorder_ = FileRecorder::CreateFileRecorder(
                _recvFileRecorderId, kFileFormatPcm16kHzFile);
            if (!recv_file_recorder_) {
                _engineStatisticsPtr->SetLastError(VE_CANNOT_START_RECORDING, kTraceError,
                    "StartRecordingRecv() CreateFileRecorder failed.");
                return -1;
            }

            if (0 != recv_file_recorder_->StartRecordingAudioFile(
                fileNameUTF8, *codecInst, notificationTime)) {
                _engineStatisticsPtr->SetLastError(
                    VE_BAD_FILE, kTraceError,
                    "StartRecordingSend() failed to start file recording");
            ...
            recv_file_recorder_->RegisterModuleFileCallback(this);
            _recvFileRecording = true;
            return 0;
        }


        int Channel::StartRecordingRecv(OutStream* stream, CodecInst* codecInst /*= NULL*/)
        {
            if (_recvFileRecording) {
                return 0;
            }
        ...
        int Channel::StopRecordingRecv()
        {
            WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, -1),
                "Channel::StopRecordingRecv()");

            if (!_recvFileRecording) {
                WEBRTC_TRACE(kTraceError, kTraceVoice, VoEId(_instanceId, -1),
                    "StopRecordingRecv() isnot recording");
                return -1;
            }

            rtc::CritScope cs(&_fileCritSect);

            if (recv_file_recorder_->StopRecording() != 0) {
                _engineStatisticsPtr->SetLastError(
                    VE_STOP_RECORDING_FAILED, kTraceError,
                    "StopRecordingRecv() could not stop recording");
                return (-1);
            }
            recv_file_recorder_->RegisterModuleFileCallback(NULL);
            recv_file_recorder_.reset();
        ...
        int Channel::StopRecordingSend()
        {
            WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, -1),
                "Channel::StopRecordingSend()");

            if (!_sendFileRecording) {
                WEBRTC_TRACE(kTraceError, kTraceVoice, VoEId(_instanceId, -1),
                    "StopRecordingSend() isnot recording");
                return -1;
            }

            rtc::CritScope cs(&_fileCritSect);

            if (send_file_recorder_->StopRecording() != 0) {
                _engineStatisticsPtr->SetLastError(
                    VE_STOP_RECORDING_FAILED, kTraceError,
                    "StopRecordingSend() could not stop recording");
                return (-1);
            }
            send_file_recorder_->RegisterModuleFileCallback(NULL);
            send_file_recorder_.reset();
        ...
        int Channel::GetSpeechRecvLevel(uint32_t& level) const
        {
            int8_t currentLevel = _recvAudioLevel.Level();
            level = static_cast<int32_t>(currentLevel);
            return 0;
        }

        int Channel::GetSpeechRecvLevelFullRange(uint32_t& level) const
        {
            int16_t currentLevel = _recvAudioLevel.LevelFullRange();
            level = static_cast<int32_t>(currentLevel);
            return 0;
        }
        ...
        uint32_t Channel::Demultiplex(const AudioFrame& audioFrame)
        {
            ...
            if (_localFileSending) {
                rtc::CritScope cs(&_fileCritSect);
                local_file_player_->Get10msAudioFromFile(
                    _audioFrame.data_, &_audioFrame.samples_per_channel_, audioFrame.sample_rate_hz_);
            } else {
...

            {
                rtc::CritScope cs(&_fileCritSect);
                if (_sendFileRecording) {
                    send_file_recorder_->RecordAudioToFile(_audioFrame);
                }
            }
            ...
            size_t number_of_channels)
        {
            ...
            voe::RemixAndResample(audio_data, number_of_frames, number_of_channels,
