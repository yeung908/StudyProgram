...
namespace webrtc
{
    VosECodec* VosECodec::GetInterface(VoiceServerEngine* VoiceServerEngine)
    {
    ...
    VosECodecImpl::VosECodecImpl(vose::SharedData* shared) : _shared(shared)
    {
        WEBRTC_TRACE(kTraceMemory, kTraceVoice, VoEId(_shared->instance_id(), -1),
            "VosECodecImpl() - ctor");
    }

    VosECodecImpl::~VosECodecImpl()
    {
        WEBRTC_TRACE(kTraceMemory, kTraceVoice, VoEId(_shared->instance_id(), -1),
            "~VosECodecImpl() - dtor");
    }

    int VosECodecImpl::NumOfCodecs()
    {
    ...
    int VosECodecImpl::GetCodec(int index, CodecInst& codec)
    {
    ...
    int VosECodecImpl::SetSendCodec(int channel, const CodecInst& codec)
    {
    ...
    int VosECodecImpl::GetSendCodec(int channel, CodecInst& codec)
    {
        if (!_shared->statistics().Initialized()) {
            _shared->SetLastError(VE_NOT_INITED, kTraceError);
            return -1;
    ...
    int VosECodecImpl::SetBitRate(int channel, int bitrate_bps)
    {
        WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
    ...
    int VosECodecImpl::GetRecCodec(int channel, CodecInst& codec)
    {
        if (!_shared->statistics().Initialized()) {
            _shared->SetLastError(VE_NOT_INITED, kTraceError);
            return -1;
    ...
    int VosECodecImpl::SetRecPayloadType(int channel, const CodecInst& codec)
    {
        WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
    ...
    int VosECodecImpl::GetRecPayloadType(int channel, CodecInst& codec)
    {
        if (!_shared->statistics().Initialized()) {
            _shared->SetLastError(VE_NOT_INITED, kTraceError);
            return -1;
    ...
    int VosECodecImpl::SetSendCNPayloadType(int channel,
        int type,
    ...
    int VosECodecImpl::SetFECStatus(int channel, bool enable)
    {
        WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
    ...
    int VosECodecImpl::GetFECStatus(int channel, bool& enabled)
    {
        if (!_shared->statistics().Initialized()) {
            _shared->SetLastError(VE_NOT_INITED, kTraceError);
            return -1;
    ...
    int VosECodecImpl::SetVADStatus(int channel,
        bool enable,
        VadModes mode,
        bool disableDTX)
    {
    ...
    int VosECodecImpl::GetVADStatus(int channel,
        bool& enabled,
        VadModes& mode,
        bool& disabledDTX)
    {
    ...
    int VosECodecImpl::SetOpusMaxPlaybackRate(int channel, int frequency_hz)
    {
        WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
    ...
    int VosECodecImpl::SetOpusDtx(int channel, bool enable_dtx)
    {
        WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
    ...
    int VosECodecImpl::GetOpusDtxStatus(int channel, bool* enabled)
    {
        WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
