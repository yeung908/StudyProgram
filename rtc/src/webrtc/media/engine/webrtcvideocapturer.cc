...
bool WebRtcVideoCapturer::Init(const Device& device) {
  ...
  for (int index = 0; index < num_cams; ++index) {
    ...
    if (info->GetDeviceName(index, vcm_name, arraysize(vcm_name), vcm_id,
                            arraysize(vcm_id)) != -1) {
      if (device.name == reinterpret_cast<char*>(vcm_name)&& (rtc::FromString<int>(device.id) == index)) {
        found = true;
        break;
      }
    }
...
void WebRtcVideoCapturer::SwitchCamera(void *handler) {
  if(module_) {
    module_->SwitchCamera(handler);
  }
}
#endif
