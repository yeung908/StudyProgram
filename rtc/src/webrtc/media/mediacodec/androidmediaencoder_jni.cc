...
#include "webrtc/media/mediacodec/androidmediaencoder_jni.h"
...
class MediaCodecVideoEncoder : public webrtc::VideoEncoder,
 ...
 private:
  ...
  rtc::ThreadChecker codec_thread_checker_;
    std::unique_ptr<NativeRegistration> j_m_encoder_class_;
  std::unique_ptr<GlobalRef>  j_m_video_encoder_;
...
MediaCodecVideoEncoder::MediaCodecVideoEncoder(JNIEnv* jni,
  ...
  // thread.
    j_m_encoder_class_ = JVM::GetInstance()->environment()->RegisterNatives(
            "org/webrtc/MediaCodecVideoEncoder", NULL, 0);
  j_m_video_encoder_ = j_m_encoder_class_->NewObject("<init>","()V");
  ...
  codec_thread_checker_.DetachFromThread();
   std::unique_ptr<NativeRegistration> j_output_buffer_info_class =
    JVM::GetInstance()->environment()->RegisterNatives(
            "org/webrtc/MediaCodecVideoEncoder$OutputBufferInfo", nullptr, 0);
  j_init_encode_method_ = j_m_encoder_class_->GetMethodId(
      "initEncode",
      "(Lorg/webrtc/MediaCodecVideoEncoder$VideoCodecType;"
      "IIIILorg/webrtc/EglBase14$Context;)Z");
  j_get_input_buffers_method_ = j_m_encoder_class_->GetMethodId("getInputBuffers", "()[Ljava/nio/ByteBuffer;");
  j_dequeue_input_buffer_method_ = j_m_encoder_class_->GetMethodId("dequeueInputBuffer", "()I");
  j_encode_buffer_method_ = j_m_encoder_class_->GetMethodId("encodeBuffer", "(ZIIJ)Z");
  j_encode_texture_method_ = j_m_encoder_class_->GetMethodId("encodeTexture", "(ZI[FJ)Z");
  j_release_method_ = j_m_encoder_class_->GetMethodId("release", "()V");
  j_set_rates_method_ = j_m_encoder_class_->GetMethodId("setRates", "(II)Z");
  j_dequeue_output_buffer_method_ = j_m_encoder_class_->GetMethodId("dequeueOutputBuffer",
      "()Lorg/webrtc/MediaCodecVideoEncoder$OutputBufferInfo;");
  j_release_output_buffer_method_ = j_m_encoder_class_->GetMethodId("releaseOutputBuffer", "(I)Z");

  j_color_format_field_ = j_m_encoder_class_->GetFieldId("colorFormat", "I");
  j_info_index_field_ = j_output_buffer_info_class->GetFieldId("index", "I");
  j_info_buffer_field_ = j_output_buffer_info_class->GetFieldId("buffer", "Ljava/nio/ByteBuffer;");
  j_info_is_key_frame_field_ = j_output_buffer_info_class->GetFieldId("isKeyFrame", "Z");
  j_info_presentation_timestamp_us_field_ = j_output_buffer_info_class->GetFieldId("presentationTimestampUs", "J");
  board_name_ = GetAndroidBoardName();
...
void MediaCodecVideoEncoder::DettachThread() {
  return codec_thread_->Invoke<void>(
          RTC_FROM_HERE, Bind(&MediaCodecVideoEncoder::DettachOnCodecThread, this));
}

void MediaCodecVideoEncoder::DettachOnCodecThread()
{
...
bool MediaCodecVideoEncoder::ProcessHWErrorOnCodecThread(
  ...
  if (reset_if_fallback_unavailable) {
    ALOGE << "Reset encoder.";
    return ResetCodecOnCodecThread();
  } else if (FindMatchingCodec(cricket::InternalEncoderFactory().supported_codecs(),
                        codec_)) {
...
int32_t MediaCodecVideoEncoder::InitEncodeOnCodecThread(
  ...
  const bool encode_status = jni->CallBooleanMethod(
      j_m_video_encoder_->GetRawObject(), j_init_encode_method_,
  ...
  if (!use_surface) {
    jobjectArray input_buffers = reinterpret_cast<jobjectArray>(
        jni->CallObjectMethod(j_m_video_encoder_->GetRawObject(),
    ...
    switch (GetIntField(jni, j_m_video_encoder_->GetRawObject(),
...
int32_t MediaCodecVideoEncoder::EncodeOnCodecThread(
  ...
  if (!input_frame.video_frame_buffer()->native_handle() ||
          MAXHUB::kMainBoardRK3399 == board_name_ ||
          MAXHUB::kMainBoardMTK5508 == board_name_) {
    int j_input_buffer_index = jni->CallIntMethod(j_m_video_encoder_->GetRawObject(),
    ...
    VideoFrame to_enc_frame = input_frame;

    if((MAXHUB::kMainBoardRK3399 == board_name_ || MAXHUB::kMainBoardMTK5508 == board_name_) &&
            input_frame.video_frame_buffer()->native_handle()) {
      rtc::scoped_refptr<VideoFrameBuffer> converted_buffer(
              to_enc_frame.video_frame_buffer()->NativeToI420Buffer());
      if (!converted_buffer) {
        LOG(LS_ERROR) << "Frame conversion failed, dropping frame.";
        return WEBRTC_VIDEO_CODEC_ERROR;
      }
      to_enc_frame = VideoFrame(converted_buffer,
                                to_enc_frame.timestamp(),
                                to_enc_frame.render_time_ms(),
                                to_enc_frame.rotation());
    }
    encode_status = EncodeByteBufferOnCodecThread(jni, key_frame, to_enc_frame,
...
bool MediaCodecVideoEncoder::EncodeByteBufferOnCodecThread(JNIEnv* jni,
  ...
  bool encode_status = jni->CallBooleanMethod(j_m_video_encoder_->GetRawObject(),
...
int32_t MediaCodecVideoEncoder::ReleaseOnCodecThread() {
  ...
  jni->CallVoidMethod(j_m_video_encoder_->GetRawObject(), j_release_method_);
  if (CheckException(jni)) {
...
int32_t MediaCodecVideoEncoder::SetRatesOnCodecThread(uint32_t new_bit_rate,
  ...
  bool ret = jni->CallBooleanMethod(j_m_video_encoder_->GetRawObject(),
                                       j_set_rates_method_,
...
bool MediaCodecVideoEncoder::DeliverPendingOutputs(JNIEnv* jni) {
  ...
  while (true) {
    jobject j_output_buffer_info = jni->CallObjectMethod(
        j_m_video_encoder_->GetRawObject(), j_dequeue_output_buffer_method_);
    if (CheckException(jni)) {
...
//    record_file_.Write(payload, payload_size);
    ...
    bool success = jni->CallBooleanMethod(j_m_video_encoder_->GetRawObject(),
...
MediaCodecVideoEncoderFactory::MediaCodecVideoEncoderFactory()
    ...
    JNIEnv* jni = GetCurrentThreadEnv();
    ScopedLocalRefFrame local_ref_frame(jni);
    std::unique_ptr<NativeRegistration> j_encoder_class = JVM::GetInstance()->environment()->
            RegisterNatives("org/webrtc/MediaCodecVideoEncoder", nullptr, 0);

    supported_codecs_.clear();

    bool is_vp8_hw_supported = jni->CallStaticBooleanMethod(j_encoder_class->GetClass(),
                                                            j_encoder_class->GetStaticMethodId("isVp8HwSupported", "()Z"));
    CHECK_EXCEPTION(jni);
  ...
  bool is_vp9_hw_supported = jni->CallStaticBooleanMethod(j_encoder_class->GetClass(),
                                                          j_encoder_class->GetStaticMethodId("isVp9HwSupported", "()Z"));
  CHECK_EXCEPTION(jni);
  ...
  if (webrtc::field_trial::FindFullName(kH264HighProfileFieldTrial) ==
    ...
    is_h264_high_profile_hw_supported = jni->CallStaticBooleanMethod(
            j_encoder_class->GetClass(),
            j_encoder_class->GetStaticMethodId("isH264HighProfileHwSupported","()Z"));
    CHECK_EXCEPTION(jni);
  }
  ...
  bool is_h264_hw_supported = jni->CallStaticBooleanMethod(
          j_encoder_class->GetClass(),
          j_encoder_class->GetStaticMethodId("isH264HwSupported", "()Z"));
  CHECK_EXCEPTION(jni);
